package com.dvhchuot.rnrecord.camera;
...
public class CameraInstance {
    ...
    private int mPreferPreviewWidth = 540;
    private int mPreferPreviewHeight = 960;
    ...
    public synchronized void startPreview(SurfaceTexture texture, Camera.PreviewCallback callback) {
        Log.e(LOG_TAG, "Camera startPreview...");
        if(mIsPreviewing) {
...
//                mCameraDevice.setPreviewCallbackWithBuffer(callback);
    ...
    public synchronized void stopPreview() {
        if(mIsPreviewing && mCameraDevice != null) {
            Log.e(LOG_TAG, "Camera stopPreview...");
            mIsPreviewing = false;
            mCameraDevice.stopPreview();
        }
    }
    ...
    public void initCamera(int previewRate) {
        ...
        for(int fmt : supportedPictureFormats) {
            Log.e(LOG_TAG, String.format("Picture Format: %x", fmt));
        }
        ...
        for(Camera.Size sz : picSizes) {
            Log.e(LOG_TAG, String.format("Supported picture size: %d x %d", sz.width, sz.height));
        ...
        for(Camera.Size sz : prevSizes) {
            Log.e(LOG_TAG, String.format("Supported preview size: %d x %d", sz.width, sz.height));
        ...
        for(Integer n : frameRates) {
            Log.e(LOG_TAG, "Supported frame rate: " + n);
            if(fpsMax < n) {
                fpsMax = n;
            }
        }
...
//        mParams.setPreviewFpsRange(20, 60);
        ...
        Log.e(LOG_TAG, String.format("Camera Picture Size: %d x %d", szPic.width, szPic.height));
        Log.e(LOG_TAG, String.format("Camera Preview Size: %d x %d", szPrev.width, szPrev.height));
    }
    ...
    public synchronized void focusAtPoint(float x, float y, float radius, final Camera.AutoFocusCallback callback) {
        ...
        if(mParams.getMaxNumMeteringAreas() > 0) {
            ...
            Log.e(LOG_TAG, "The device does not support metering areas...");
            try {
                mCameraDevice.autoFocus(callback);
            } catch (Exception e) {
                Log.e(LOG_TAG, "Error: focusAtPoint failed: " + e.toString());
